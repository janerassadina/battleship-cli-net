...
- **Game Summary:** Each player gets five ships of different sizes and secretly places them on a hidden 10x10 grid. Players then take turns guessing coordinates on the opponent's grid, attempting to locate and sink all of the opponent's ships before the opponent sinks theirs. Ships must be placed horizontally or vertically and cannot overlap. 

### 1.2 Core Game Rules & Win/Draw Conditions
- **Turn Mechanics:** Players place their ships first. Once both fleets are places, the server randomly selects which player goes first. After that, turns alternate: the active player guesses a coordinate. If it's a hit, they go again. If it's a miss, the turn passes to the other player. One full turn (until a miss) counts as one round for that player. 
- **Victory Condition:** If a player fully sinks all 5 of the opponent's ships (every cell of every ship hit) during their turn, the game isn't decided yet - the opponent still gets to take their turn for that same round. If the opponent doesn't also sink the entire fleet during that turn, the first player to have sunk all ships wins.  
- **Draw/Tie Condition:** If both players end up sinking all of the opponent's ships within the same round, the game is a tie.
